...
def save_result(
    ...
    if args.umap_plot:
        ...
        sc.settings.figdir = 'results/figures/'  # Set the figure directory
        try:
            target_annotation = target_adata.obs["Group"]
        except:
            target_annotation = target_adata.obs["Prediction"]
        adata.obs["Group"] = np.concatenate(
            (source_adata.obs["Group"], target_annotation), axis=0
        )
        ...
        sc.pl.umap(
            ...
            palette=sns.color_palette(
                "husl", np.unique(adata.obs["Group"].values).size
            ),
            save="_" + args.data_path[:-1] + "_Group.png",
            show=False,
        )
        sc.pl.umap(
            adata,
        ...
        # Removed the code that saves the Reliability UMAP plot
        # sc.pl.umap(
        #     target_adata,
        #     color=["Reliability"],
    ...
    try:
        ...
        print("No Target Group Annotations Provided, Skip Evaluation")
        evaluation = False
    if evaluation and not args.novel_type:
        ...
        if args.umap_plot:
            sil_type = silhouette_score(
                adata.obsm["X_umap"], adata.obs["Group"].values
            )
    ...
    # Update the save paths to point to '/results/' directory
    source_save_path = 'results/' + args.source_data[:-4] + "-integrated.h5ad"
    target_save_path = 'results/' + args.target_data[:-4] + "-integrated.h5ad"
    all_save_path = 'results/' + "all-integrated.h5ad"

    source_adata.write(source_save_path)
    target_adata.write(target_save_path)
    adata.write(all_save_path)
    print(
