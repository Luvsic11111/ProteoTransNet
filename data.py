...
def prepare_dataloader(args):
    df = pd.read_csv(args.data_path + args.source_data,dtype={'Domain':str,'Group':str})
    obs = df[['Domain', 'Group']]
    expr_data = df.drop(['Domain', 'Group'], axis=1)
    source_adata = ad.AnnData(X=expr_data, obs=obs)
    ...
    source_adata.obs["Domain"] = args.source_data[:-4]
    source_label = source_adata.obs["Group"]
    ...
    df1 = pd.read_csv(args.data_path + args.target_data)
    obs1 = df1[['Domain', 'Group']]
    expr_data1 = df1.drop(['Domain', 'Group'], axis=1)
    target_adata = ad.AnnData(X=expr_data1, obs=obs1)
    ...
    target_adata.obs["Domain"] = args.target_data[:-4]
    ...
    protein_num = source_data.shape[1]
